...
public final class DataSourceIncrementalService {
    ...
    public static void processIncrementalIndexing( DataSource dataSource, List < IndexerAction > indexerActionList, int nIdTask ) throws ElasticClientException {
        ...
        List <String> idDataObjectList = indexerActionList.parallelStream( ).map( indexerAction -> IndexerAction.TASK_DELETE == nIdTask ? DataSourceService.getIdDocument( dataSource.getId( ), indexerAction.getIdResource( ) ) : indexerAction.getIdResource( ) ).collect( Collectors.toList( ) );
